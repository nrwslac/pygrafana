...
def update_alert(alert_uid, annotations=None, title=None, pending=None):
...
def SetupArgumentParser():
    ...
    parser.add_argument('-p','--pending', dest='pending', help="Pending time for alert", choices=["2m","5m", "10m", "1hr", "2hr", "5hr", "1d"], default=None)
...
def main(argv):
    ...
    if options.get_alert:
        ...
        if options.alert_id == None:
            ...
            exit    
        if options.update_alert == "annotations":
            required_args = ["annotations"]
            missing =  [arg for arg in required_args if getattr(options, arg) is None]
            if missing:
                print(f"To update annotations, --{required_args} is required")
                exit
            try:
                ...
                exit(1)
        elif options.update_alert == "title":
            required_args = ["title"]
            missing =  [arg for arg in required_args if getattr(options, arg) is None]
            if missing:
                print(f"To update title, --{required_args} is required")
                exit
            update_alert(options.alert_id, title=options.title) 
        elif options.update_alert == "pending":
            required_args = ["pending"]
            missing =  [arg for arg in required_args if getattr(options, arg) is None]
